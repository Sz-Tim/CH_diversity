...
The model parameterized with both the presence-only and structured abundance datasets (\emph{Joint} model) outperformed the model parameterized with only the structured abundance dataset (\emph{Structured} model) in predicting novel plot communities (Cross-validation; \emph{Joint} $D^2$=0.079; \emph{Structured} $D^2$=0.050). In the full model, however, the \emph{Structured} models fit the data somewhat more closely (Full models; \emph{Joint} $D^2$=0.19; \emph{Structured} $D^2$=0.21). This same pattern occurred on average across species (Full model; \emph{Joint} $D^2_s$ median: 0.17; \emph{Structured} $D^2_s$ median: 0.23). Across plots, there was no clear elevational pattern in $D^2_i$ (\textbf{Appendix 2 Fig. X}; Full models; \emph{Joint} $D^2_1$ median: 0.12; \emph{Structured} $D^2_i$ median: 0.15).
...
Patterns of $\beta$-diversity and its components did not strongly differ between models (Fig. \ref{fig:beta_div}). Overall $\beta$-diversity among plots within each structured site showed a very modest peak at intermediate elevations (\emph{Joint}: p=0.02, R$^2$=0.14, $\Delta$AICc=0.7; \emph{Structured}: p=0.009, R$^2$=0.17, $\Delta$AICc=3.3). The \emph{balanced variation} component drove variation among local plots at lower elevations, indicating changes in the relative intensity of species, and decreased at higher elevations (\emph{Joint}: p$<$0.001, R$^2$=0.70, $\Delta$AICc=5.4; \emph{Structured}: p$<$0.001, R$^2$=0.49, $\Delta$AICc=0.08). At higher elevations, the \emph{abundance gradient} component increased, indicating changes in total intensity among plots, with less variation in species' relative intensity (\emph{Joint}: p$<$0.001, R$^2$=0.72, $\Delta$AICc=10.6; \emph{Structured}: p$<$0.001, R$^2$=0.44, $\Delta$AICc=2.4). 
