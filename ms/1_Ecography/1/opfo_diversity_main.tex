...
Citizen science has become quite common recently, and samples collected by citizen scientists can represent a large component of ecological datasets \citep{Poisson2020}. It's useful for conservation \citep{Duan2020} and monitoring invasive species \citep{Roy-Dufresne2019,Pernat2020}. The goals and designs of citizen science projects are of course extremely diverse, but they have been increasingly used for presence-based species distribution models. In many cases, the performance of SDMs fit with citizen science data can be equivalent to that of SDMs fit with scientifically collected data \citep{Steen2019,Sumner2019,Henckel2020}. 

% Potential for CS to inform under-represented taxa
% include short-comings (ID - easy remedy for insects)
The aid of citizen scientists can help to overcome logistical limitations common in ecology. The large number of volunteer collectors can cover a broad spatial extent \citep{Theobald2015}, and include private land that is otherwise inaccessible to researchers \citep{Pernat2020}. They can be spatially biased \citep{Isaac2014,Johnston2020}, and using them requires careful thought and accounting for biases or particularities of their collection \citep{Steen2019,Robinson2020}. Effort has not been equal across taxonomic groups \citep{Theobald2015,Troudet2017}. Citizen science could be particularly useful for overcoming the general shortfall in knowledge of insect distributions, particularly since specimens can be collected and easily returned for expert identification or genetic analysis.

% Modelling richness with presence-only data: iSDMs, jSDMs, MRMs, mSDMs
Because the survey-type citizen science projects collect a broad set of species, it is natural to try to use them to estimate community-level metrics like richness. There are a variety of methods, including those that model richness directly and those developed from methods for species distribution models \citep{Dubuis2011,Guisan2011,Calabrese2014,Biber2019}. These include stacking independent SDMs fit for each species, and modelling species' responses jointly or in a hierarchical framework \citep{Caradima2019}. Similarly, multi-species occupancy models can be used, particularly when the data are presence-absence rather than presence-only \citep{Frishkoff2019,Guillera-Arroita2019,Szewczyk2018,Devarajan2020}. However, occupancy models require multiple sampling bouts to estimate detection probabilities, and consequently their use is limited to sampling designs with repeated samples.

% Methods for merging datasets
There is evidence that predictions based on more structured datasets, including designs with presence-absence or repeatedly sampled sites, can be improved by incorporating the additional information provided by presence-only datasets such as those collected by citizen scientists \citep{Dorazio2014b,Fithian2015,Koshkina2017a}. There are some complications and best practices \citep{Fletcher2019}. Bayesian methods naturally and flexibly accommodate the incorporation of multiple datasets \citep{Clark2005,BeckEtAl2012,Szewczyk2018}, and several distinct approaches have been outlined in the context of modelling species distribution using different data types \citep{Fletcher2019,Isaac2019,Miller2019}. Past work has used multiple datasets to connect spatial scales via downscaling \citep{Keil2013a,Keil2014}.

% Poisson point process models to unite spatial scales and accommodate density/abundance data
Inhomogenous Poisson Point Process Models (PPMs) model a continuous intensity that varies across space, making them a natural choice for modelling the variation in species distributions or suitability across landscapes, and in fact they can be mathematically equivalent to common presence-only SDM methods \citep{Renner2013a,Renner2015}. They are particularly appealing for combining different data sets, because the continuous process can integrate to different spatial resolutions, and observations collected with different methods can be modelled using different sampling submodels based on the same underlying intensities \citep{Fithian2015,Hefley2016,Koshkina2017a,Fletcher2019,Renner2019}. The treatment of spatial scale is different compared to \citep{Fithian2015}, which models spatial bias in the presence-only observations and assumes a single regression.
...
Here, we combine species occurrences of ants collected in a citizen science project in western Switzerland with species abundance data from a concurrent structured sampling effort. At its core, this model is centered around local density data, with additional presence-only information added at a regional scale. In a hierarchical Bayesian framework, we use the citizen science data to help inform species' responses to regional variables (because local environmental variables are unavailable), and the structured samples to inform responses to both regional and local variables, while accounting for bias in geographic and taxonomic sampling effort in the citizen science data. With this model, we detail the patterns of ant colony density and species diversity across the landscape while incorporating uncertainty in species compositions, and we evaluate the support for hypothesized drivers across spatial scales. We compare inferences from the combined model with those from each dataset independently, and also assess the differences in observed communities from each sampling method. 
