...
data {
  ...
  matrix<lower=0>[I_,S] Y_;  // Y counts (test)
...
generated quantities {
  ...
  for(s in 1:S) {
    for(i in 1:I_) {
      log_lik_[i,s] = GP_point_log_lpdf(Y_[i,s] | disp_lam, llambda_[i,s]);
      prPresL_[i,s] = 1 - exp(GP_point_log_lpdf(0 | disp_lam, llambda_[i,s]));
    }
  }
